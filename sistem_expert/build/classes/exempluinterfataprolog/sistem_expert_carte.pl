...
setof(str(FC, av(Atr, Val)), R^(fapt(av(Atr, Val), FC, R), FC >= 20), L), !,
afiseaza_scopuri(Stream, L), nl(Stream), flush_output(Stream).

afiseaza_scop(Stream, _) :- format(Stream, 's(~p)', ['Ne pare rau, nu s-au gasit solutii']), nl(Stream), flush_output(Stream).
...
realizare_scop(av(Atr, nu_conteaza), FC, _) :- 
...
interogheaza(Atr, Mesaj, Optiuni, Istorie) :-
	...
	constrangeri(L),
	(member(Atr, L) ->
		append(Optiuni, [nu_conteaza, nu_stiu], Optiuni1);
		Optiuni1 = Optiuni),
...
see(R), incarca_solutii, seen, see(C), incarca_constrangeri, seen, see(F), incarca_reguli, seen, !.
